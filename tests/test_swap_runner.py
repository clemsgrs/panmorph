...
from panmorph.swap_runner import estimate_swap_cells, run_swap_bundle, validate_swap_bundle
...
def test_swap_runner_emits_the_complete_configured_grid(swap_bundle: Path) -> None:
    assert len(_rows(swap_bundle / "swap_aucs.csv")) == 12
    assert len(_rows(swap_bundle / "swap_summaries.csv")) == 6
    assert len(_rows(swap_bundle / "swap_predictions.csv")) == 1_200
...
def test_swap_runner_records_the_monotone_target_reference(swap_bundle: Path) -> None:
    assert _rows(swap_bundle / "swap_reference.csv") == [
        {"cases": "0.0", "origin": "e1", "raw_auc": "0.5", "monotone_auc": "0.5"},
        {"cases": "10.0", "origin": "swap", "raw_auc": "1.0", "monotone_auc": "1.0"},
        {"cases": "20.0", "origin": "swap", "raw_auc": "1.0", "monotone_auc": "1.0"},
        {"cases": "40.0", "origin": "e1", "raw_auc": "1.0", "monotone_auc": "1.0"},
        {"cases": "80.0", "origin": "e1", "raw_auc": "1.0", "monotone_auc": "1.0"},
    ]
...
def test_swap_runner_bootstraps_conditional_equivalence(swap_bundle: Path) -> None:
    row = next(
        ...
        if (row["budget"], row["target_share"]) == ("10", "50")
    )

    assert row == {
        "budget": "10", "target_share": "50", "source_cases": "5",
...
def test_swap_estimate_bootstraps_raw_and_rank_auc() -> None:
    predictions = tuple(
        SwapPredictionRecord(
            10, 50, draw, fold, (f"S{fold}",), case_id, label, score
        )
        for draw in (0, 1)
    ...
    aucs = tuple(
        SwapAucRecord(10, 50, draw, 1.0, 1.0, 0.0, False)
        for draw in (0, 1)
    )
