...
## The same test with PRISM2 features

We repeated the gate and the few-label bundle with PRISM2 slide embeddings in both output
formats, `prism2-base` and `prism2-diagnostic`. The probe, the splits, the draws, the
registered cell, and its rule are unchanged. The zero-shot verdict matrix below is
generated from the committed gate tables.
...
The registered cell per feature set (colon data for the stomach model, ten local
MSI-positive cases, single source):
...
The rule is met for `prism2-base` only. No test between feature sets is computed, and no
feature set is declared the winner. The probe is unchanged, so a wider embedding at the same regularization strength is effectively less regularized.
The PRISM2 bundles are in `../prism2-base/few-label/` and `../prism2-diagnostic/few-label/`.
